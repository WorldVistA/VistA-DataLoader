...
namespace DataLoader
{
    ...
    {
        //string file = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "DataLoader.ini");
        IniFile ini = new IniFile();

        public Form_Connect()
        {
            InitializeComponent();
            server.Text = ini.IniReadValue("server", "value");
            port.Text = ini.IniReadValue("port", "value");
        }
        ...
        private void server_TextChanged(object sender, EventArgs e)
        {
            ...
            //ini.IniWriteValue("server", "value", server.Text);
        }

        private void port_TextChanged(object sender, EventArgs e)
        {
            //ini.IniWriteValue("port", "value", port.Text);
            button1.Enabled = true;
        }
